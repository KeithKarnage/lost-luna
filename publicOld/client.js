...
//21
    ...
    cmdSets = [
        //  MARK 1 BODY
        [[0,[[6,3,3],[6,7,4]],[[0,16,7,16],[16,-1,8,16,2,16]],[2,3]]],
        //  MINI BOMB BODY
        [[0,[[6,8,3]],[[0,16,7,16],[16,-1,8,16,2,16]],[2]]],
        //  LEGS
        [
            [1,[[6,11,2]],[[0,16,7,16]]],
            [2,[[24,10,2],[40,73,2]]],
            ...
            [4,[[21,26],[21,42],[37,57]]],
            [5,[[38,26],[38,42],[22,57]]],
            [6,[[21,90],[21,106],[37,121]]],
            [7,[[38,90],[38,106],[22,121]]]
        ],
        //  3  -  EYES
        [
            [8,[[6,4]],o1],
        ...
        //  10  -  CRATERS
        [
            [33,[[3,2,1,,4],[3,10,1,,4],[3,18,2,,4],[3,26,2,,5]],,,[,19]],
            [33,[[1,2,1],[1,10,1],[1,18,2],[1,26,2]],,,[,14]],
            [33,[[2,2,1,,4],[2,10,1,,4],[2,18,2,,4],[2,26,2,,5]],,,[,12]],
        ]
    ]

let px;
function exctCmd(cs,cn,pl,X=0,Y=0) {
    ...
    cs.forEach( s => {
        ...
        if(s[3]) { s[3].forEach( (s3,i) => exctCmd(s[3][i],cn,pl,X,Y)) }
        // if(m) exctCmd(m,cn,pl,X,Y)
    })
...
function copy(cnv,sx,sy,sw,sh,x,y,w,h,e,V=0,vOff,H=0,hOff) {
    ...
    function makeSpriteSheet(C) {
        ...
        //  MARK 1
        exctCmd(0,C,[,0,1,7],48);
        exctCmd(4,C,[,11,12],48);

        //  GRENADIER
        exctCmd(0,C,[,0,1,7],48,128);

        //  NINJA
        exctCmd(0,C,[,1,3,9],96);
        exctCmd(4,C,[,11,12],96);


        //  MINI BOT
        exctCmd(1,C,[,0,2,7],0,128);
        exctCmd(3,C,[,0,2,9],0,133);

        //  MINI TOWER
        exctCmd(7,C,[,0,2,1,3],144);
        

        //  MEGA TOWER
        exctCmd(8,C,[,0,1,2,3],160);
    ...
    //  EXPLOSION
    exctCmd(9,S[0],[,17,13,18,10,14,3],176);

    //  CRATERS
    exctCmd(10,S[0],[,17,13,18,10,14,3],176,144);
    
    //  CARD BACKS
    exctCmd(6,S[0],[,11,13,14,15,16],192);
    ...
    copy(S[0],198,42,16,12,197,100,16,12,0,1);
    ...
    for(i=0; i<40; i++) {
        ctx.drawImage(S[0],
            ...
            288 + rInt(4)*16,
            32,16,
            Math.random()*L/scl,Math.random()*L/scl,32,16);
    }
    // ctx.drawImage(S[0],0,0);
    ctx.restore();

    // ctx.save();
    //     ctx.translate(-512,-364)
    //     ctx.scale(2,2);
